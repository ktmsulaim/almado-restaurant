    ...
    <div class="content container-fluid">
        ...
        <div class="card mb-3 mb-lg-5 mt-2">
            <div class="card-header">
                <h4 class="page-header-title">{{$dm['f_name'].' '.$dm['l_name']}}@if($dm['status']) (@if($dm->zone) {{$dm->zone->name}} @else {{__('messages.zone').' '.__('messages.deleted')}} @endif ) @if($dm['active']) <label class="badge badge-soft-primary">{{__('messages.online')}}</label> @else <label class="badge badge-soft-danger">{{__('messages.offline')}}</label> @endif  @else <span class="badge badge-danger">{{__('messages.suspended')}}</span> @endif</h4>
                ...
                <a  href="javascript:"  onclick="request_alert('{{route('admin.delivery-man.status',[$dm['id'],$dm->status?0:1])}}','{{$dm->status?'Want to suspend this deliveryman ?':'Want to unsuspend this deliveryman'}}')" class="btn {{$dm->status?'btn-danger':'btn-success'}}">
                        ...
                        <tr>
                            ...
                            <td>
                                @foreach(json_decode($review['attachment'],true) as $attachment)
                                    <img width="100" onerror="this.src='{{asset('assets/admin/img/160x160/img2.jpg')}}'" src="{{asset('storage/public')}}/{{$attachment}}">
                                @endforeach
                            </td>
                            <td>
